...
1. Fail closed when any file operation encounters a symlink or Windows junction/reparse point, including a redirected root or existing parent component.
...
| Windows junction/reparse | Treat as a symlink: reject the named path and do not follow |
| Root symlink | Reject |
...
| Non-local relative | Reject `!filepath.IsLocal`, reserved device names (`NUL`/`CON`/`COM1`), rooted-backslash, and drive-relative/volume-qualified relatives as `PATH_ESCAPE` |
...
- `ValidateNoSymlinkComponents(path string) error`
  ...
  - Return `SYMLINK_DISALLOWED` naming the first symlink, Windows junction, or other reparse point encountered. Prefer a precise reparse attribute on Windows; if attributes cannot be read, fail closed on `ModeIrregular`.
...
- `SafeJoin(root, relative string) (string, error)`
  ...
  - Reject `..`, any cleaned path beginning with `..` plus a separator, and any value that is not `filepath.IsLocal`.
  - Reject reserved device names (`NUL`, `CON`, `COM1`, and the rest of the Windows reserved set), rooted-backslash paths, and drive-relative or volume-qualified relatives.
  - Join to an absolute, cleaned root.
  - Re-check with `filepath.Rel(root, joined)` and reject escape. Wrap `filepath.Rel` failures as `PATH_ESCAPE`.
...
2. `filepath.Walk` already obtains entries with `Lstat`; if an entry is a symlink or Windows junction/reparse point, return `SYMLINK_DISALLOWED` before invoking the caller callback.
3. An encountered symlink or reparse point aborts counting, preflight, encryption, decryption, or secure deletion consistently.
...
- `ValidateNoSymlinkComponents`: regular path, symlink leaf, symlink parent, missing output leaf, filesystem error, Windows junction/reparse when creation is available.
- `SafeJoin`: normal nested path, absolute path, `..`, nested `../`, sibling prefix trap, symlinked root, symlinked parent, reserved device names, non-local relatives, rooted-backslash, and drive-relative/volume-qualified relatives.
...
On Windows, tests attempt `os.Symlink` and junction creation; skip only when creation itself reports unavailable privilege/support. Unexpected creation errors fail the test. Containment tests run everywhere.
...
- [ ] No supported command follows a symlink, Windows junction, or other reparse input, or writes through a redirected output component.
- [ ] Directory output construction rejects absolute, parent-escaping, non-local, reserved-device, rooted-backslash, and volume-qualified relative paths.
